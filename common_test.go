...
func TestUse(t *testing.T) {
	...
	ctx, ok := s1.(*useCtx)
	a.True(ok)
	a.Equal("test", ctx.CAS.Username)
	a.Equal("test", ctx.CAS.Password)
	a.Equal(casBaseURL, ctx.CAS.BaseURL)
	a.Equal(casDomain, ctx.CAS.Domain)
	...
	a.Equal("test", ctx.CAS.Token)
	a.Equal(true, ctx.CAS.UseToken)
	...
	ctx, ok = s3.(*useCtx)
	a.True(ok)
	a.Equal(false, ctx.CAS.UseToken)
	a.Equal("test", ctx.CAS.Username)
	a.Equal("test", ctx.CAS.Password)
	a.Equal(webvpnBaseURL, ctx.CAS.BaseURL)
	a.Equal(webvpnDomain, ctx.CAS.Domain)
}
